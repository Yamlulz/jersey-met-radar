...
def radar_live(config: Dict[str, Any], config_dir: Path, force: bool) -> int:
    ...
    _, wide_digest = download_sequence(
        base_url, wide_dir,
        str(wide["prefix"]), str(wide["ext"]), int(wide["slots"]),
        log_path
    )
    _, zoom_digest = download_sequence(
        base_url, zoom_dir,
...
def radar_archive_24h(config: Dict[str, Any], config_dir: Path, force: bool) -> int:
    ...
    # NEW: don’t build until we have at least N “latest token” frames
    min_frames_to_build = int(pl.get("min_frames_to_build", 2))
    if min_frames_to_build < 2:
        min_frames_to_build = 2
    ...
    # Prepare tmp sequences (latest token only)
    ...
    log_line(
        log_path,
        f"24h tmp frame counts: wide={wide_count}, zoom={zoom_count}, min_required={min_frames_to_build}"
    )

    # NEW: don’t build until we have enough frames
    if (wide_count < min_frames_to_build) and (zoom_count < min_frames_to_build):
        log_line(
            log_path,
            f"Not enough frames yet to build (need >= {min_frames_to_build}). Skipping."
        )
        return 0
    ...
    if wide_count >= min_frames_to_build:
        ok_wide = build_from_tmp(wide_tmp, out_wide, "WIDE 24h")
    else:
        log_line(log_path, f"Wide has only {wide_count} frames; need {min_frames_to_build}. Not building wide.")

    if zoom_count >= min_frames_to_build:
        ok_zoom = build_from_tmp(zoom_tmp, out_zoom, "ZOOM 24h")
    else:
        log_line(log_path, f"Zoom has only {zoom_count} frames; need {min_frames_to_build}. Not building zoom.")
    ...
    # Only mark the hour as “built” if we actually produced something.
    if ok_wide or ok_zoom:
